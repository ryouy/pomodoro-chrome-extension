...
const els={timerView:$('timerView'),settingsView:$('settingsView'),settingsToggle:$('settingsToggle'),phaseLabel:$('phaseLabel'),setLabel:$('setLabel'),timeDisplay:$('timeDisplay'),start:$('startPauseButton'),pause:$('pauseButton'),reset:$('resetButton'),skip:$('skipButton'),form:$('settingsForm'),work:$('workMinutes'),break:$('breakMinutes'),sets:$('totalSets'),alarm:$('alarmSound'),bird:$('birdSound'),sound:$('soundEnabled'),notifications:$('notificationsEnabled'),notificationStatus:$('notificationStatus'),testSound:$('testSound'),testBird:$('testBird')};
...
function complement(hex){return `#${(0xffffff-Number.parseInt(hex.slice(1),16)).toString(16).padStart(6,'0')}`}
function applyTheme(name){const values=THEMES[name]||THEMES.alpine;['--accent','--dark','--break','--soft','--line','--ink','--muted'].forEach((key,index)=>document.documentElement.style.setProperty(key,values[index]));document.documentElement.style.setProperty('--complement',complement(values[0]));document.querySelector('meta[name="theme-color"]').content=values[0]}
...
function render(){
  ...
  els.start.textContent=timer.status==='running'?(timer.phase==='work'?'作業中':'休憩中'):timer.status==='complete'?'もう一度':timer.status==='paused'?'再開':'スタート';els.start.disabled=timer.status==='running';els.pause.disabled=timer.status!=='running';els.pause.classList.toggle('work-active',timer.status==='running'&&timer.phase==='work');els.skip.disabled=timer.status==='complete';
...
async function testNotification(){els.notificationStatus.textContent='';if(!els.notifications.checked)return;if(!('Notification'in window)){els.notifications.checked=false;els.notificationStatus.textContent='非対応です';return}const permission=Notification.permission==='default'?await Notification.requestPermission():Notification.permission;if(permission!=='granted'){els.notifications.checked=false;els.notificationStatus.textContent='通知がブロックされています';return}new Notification('通知テスト',{body:'タイマーの通知は正常に設定されています。',icon:'icon.svg'});els.notificationStatus.textContent='テスト送信済み'}

async function announce(completedPhase,completedSet,isComplete){if(settings.soundEnabled&&!isComplete){if(completedPhase==='work')await playBird(settings.birdSound);else await playStartSound(settings.alarmSound)}if(isComplete)notify('ポモドーロ完了',`${settings.totalSets}セット完了しました。`);else if(completedPhase==='work')notify('作業時間が終了しました',`セット ${completedSet}/${settings.totalSets} 完了。休憩を始めます。`);else notify('休憩が終了しました',`セット ${completedSet+1}/${settings.totalSets} の作業を始めます。`)}
...
els.start.addEventListener('click',async()=>{if(timer.status==='running')return;if(settings.soundEnabled)await playStartSound(settings.alarmSound);if(timer.status==='complete')timer=initialTimer();const seconds=Math.max(1,timer.remainingSeconds||duration());timer={...timer,status:'running',remainingSeconds:seconds,endTime:Date.now()+seconds*1000};save();render()});
els.pause.addEventListener('click',async()=>{if(timer.status!=='running')return;timer={...timer,status:'paused',remainingSeconds:remaining(),endTime:null};save();render();if(settings.soundEnabled)await playPauseBeep()});
...
els.testSound.addEventListener('click',()=>playStartSound(els.alarm.value));els.testBird.addEventListener('click',()=>playBird(els.bird.value));
els.notifications.addEventListener('change',testNotification);
